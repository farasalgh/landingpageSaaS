...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadOrders() {
        try {
            ...
            if (orders.length === 0) {
                ordersList.innerHTML = '<tr><td colspan="6" class="p-4 text-center">Tidak ada pesanan.</td></tr>';
                return;
            }
            ...
            orders.forEach(order => {
                ...
                row.innerHTML = `
                    ...
                    <td class="p-4">
                        <select class="status-dropdown bg-gray-700 rounded p-1" data-id="${order.id}">
                    ...
                    <td class="p-4">${new Date(order.order_date).toLocaleDateString()}</td>
                    <td class="p-4">
                        <button class="text-red-500 hover:underline delete-btn" data-id="${order.id}">Hapus</button>
                    </td>
                `;
