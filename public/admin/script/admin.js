...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadProducts() {
    try {
      ...
      if (allProducts.length === 0) {
        productList.innerHTML =
          '<tr><td colspan="4" class="p-4 text-center">Belum ada produk.</td></tr>';
        return;
      }
      ...
      allProducts.forEach((product) => {
        ...
        row.innerHTML = `
            ...
            <td class="p-4 hidden sm:table-cell">${product.stock}</td>
            <td class="p-4 text-right">
                <button class="text-blue-400 hover:underline btn btn-edit text-sm" data-id="${product.id}">Edit</button>
                <button class="text-red-500 hover:underline btn btn-delete text-sm ml-2" data-id="${product.id}">Hapus</button>
            </td>
        `;
      ...
      productList.innerHTML =
        '<tr><td colspan="4" class="p-4 text-center text-red-500">Gagal memuat produk.</td></tr>';
    }
  }
  ...
  productForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          id ? "Gagal mengupdate produk." : "Gagal menambah produk."
        );
      }
      resetForm();
      loadProducts();
